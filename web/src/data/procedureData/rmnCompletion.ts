import { rmnBatches, rmnProcedures } from "./rmn";

const QUIZ_PROGRESS_PREFIX = "nct-quiz-progress-";

export type RmnBatchProgress = {
  batchNumber: number;
  total: number;
  completed: number;
  remaining: number;
...
function completedIds(storage: Storage | null): Set<string> {
  ...
  for (const procedure of rmnProcedures) {
    try {
...
export function getRmnCompletion(storage?: Storage): RmnCompletion {
  const completed = completedIds(getStorage(storage));
  const totalProcedures = rmnProcedures.length;
  const totalQuestions = rmnProcedures.reduce(
    (total, procedure) => total + (procedure.quiz?.length ?? 0),
    0,
  );
  const batches: RmnBatchProgress[] = rmnBatches.map((batch, index) => {
