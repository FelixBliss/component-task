import { phnBatches, phnProcedures } from "./phn";

const QUIZ_PROGRESS_PREFIX = "nct-quiz-progress-";

export type PhnBatchProgress = {
  batchNumber: number;
  total: number;
  completed: number;
  remaining: number;
...
function completedIds(storage: Storage | null): Set<string> {
  ...
  for (const procedure of phnProcedures) {
    try {
...
export function getPhnCompletion(storage?: Storage): PhnCompletion {
  const completed = completedIds(getStorage(storage));
  const totalProcedures = phnProcedures.length;
  const totalQuestions = phnProcedures.reduce(
    (total, procedure) => total + (procedure.quiz?.length ?? 0),
    0,
  );
  const batches: PhnBatchProgress[] = phnBatches.map((batch, index) => {
